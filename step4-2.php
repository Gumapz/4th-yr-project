...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_booking'])) {
    ...
    $rental_duration = htmlspecialchars($_POST['day']); // Number of days

    $pickup_time_12hr = date('h:i A', strtotime($pickup_time));
    $return_time_12hr = date('h:i A', strtotime($return_time));
    ...
    $total_price2 = $rental_duration * $car_rental_fee + $extra_price; // Ensure you calculate it here
    ...
    if (empty($errors)) {
        ...
        if ($stmt) {
            ...
            $stmt->bind_param(
                ...
                $car_name,
                $car_model,
                $total_price2, // Make sure total_price is a float
                $pickup_date,
                $return_date,
                $pickup_time_12hr,
                $return_time_12hr,
            ...
            if ($stmt->execute()) {
                ...
                $message .= "<li><strong>Price:</strong> ₱" . number_format($total_price2, 2) . "</li>";
                $message .= "<li><strong>From:</strong> $pickup_date</li>";
                $message .= "<li><strong>Pickup Time:</strong> $pickup_time_12hr</li>";
                $message .= "<li><strong>Until:</strong> $return_date</li>";
                $message .= "<li><strong>Drop Time:</strong> $return_time_12hr</li>";
...
<head>
    ...
    <style>
        ...
        /* Close Button positioned in the top right corner of the modal box */
        .close-btn {
            position: absolute;
            top: 10px;
            right: 20px;
...
<body>
        ...
        <div class="content-container">
            ...
            <div class="booking-details-box1">
                <div class="booking-details">
                    ...
                    <input type="hidden" name="return_time" value="<?= $pickup_time; ?>">
                    <input type="hidden" name="day" value="<?= $day; ?>">

                        <p>Pick-up Date: <?= htmlspecialchars($pickup_date); ?></p>
                        <p>Pick-up Time: <?= htmlspecialchars($pickup_time_12hr); ?></p>
                        <p>Return Date: <?= htmlspecialchars($return_date); ?></p>
                        <p>Return Time: <?= htmlspecialchars($pickup_time_12hr); ?></p>
    ...
    <section class="personal-details-section">
        <div class="details-box">
            ...
            <div class="form-group1">
                ...
                <img id="selfie" alt="Captured Selfie" style="display:none; margin-top: 10px; width: 50%; border-radius: 5px;"/>
    ...
    <div class="form-group1">
        <label>
            ...
            I have read and agree to the <a href="javascript:void(0);" onclick="openModal()" style="color: blue; text-decoration: underline;">rental terms</a>
        </label>
    </div>
</div>
...
<div id="rentalTermsModal" class="modal">
    <div class="modal-content">
        ...
        <h2 class="modal-title">Rental Terms</h2>
        <div class="terms-content">
            <h3>1. Eligibility Requirements:</h3>
            <p>Drivers must be at least 21 years old.</p>
            <p>A valid driver's license and a credit card are required at the time of rental.</p>

            <h3>2. Rental Period:</h3>
            <p>The rental period is calculated on a 24-hour basis.</p>
            <p>Late returns may incur additional charges.</p>

            <h3>3. Insurance and Liability:</h3>
            <p>Rental rates include basic insurance coverage.</p>
            <p>Additional insurance options are available for an extra fee.</p>

            <h3>4. Fuel Policy:</h3>
            <p>Vehicles must be returned with the same fuel level as at pickup.</p>
            <p>A refueling fee may apply if the vehicle is returned with less fuel.</p>

            <h3>5. Mileage Limit:</h3>
            <p>Rentals come with a daily mileage limit of [XX] miles/kilometers. Additional mileage charges may apply.</p>

            <h3>6. Payment and Deposit:</h3>
            <p>Full payment is required at the time of rental.</p>
            <p>A refundable security deposit will be held until the vehicle is returned.</p>

            <h3>7. Cancellations and Refunds:</h3>
            <p>Cancellations made 24 hours prior to pickup are eligible for a full refund.</p>
            <p>Cancellations within 24 hours of pickup may incur a cancellation fee.</p>
        </div>
    </div>
</div>
...
navigator.mediaDevices.getUserMedia({ 
    ...
    .catch((error) => {
        ...
        alert('Unable to access the camera. Please allow camera access or use a compatible browser.');
    });
...
function openModal() {
    ...
    window.onclick = function(event) {
        var modal = document.getElementById("rentalTermsModal");
